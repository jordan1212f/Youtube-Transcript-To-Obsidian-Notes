...
/* The reveal action card — a focus-hero at full structure with a live 48h
   countdown that's already running. Used at the end of onboarding. */
function RevealActionCard({ goalLabel }) {
    ...
    <article className="focus-hero focus-phase-green" style={{ '--phase-color': phaseColor }}>
      <div className="focus-hero-top">
        ...
        <div className="focus-hero-meta mono">
          From <em>Slow productivity</em>{goalLabel ? ` · ${goalLabel}` : ''}
        </div>
      </div>
      ...
      <div className="focus-progress">
        <div className="focus-progress-track">
          <div className="focus-progress-fill" style={{ width: `${pct * 100}%`, background: phaseColor }}></div>
        </div>
        ...
        <div className="chain-track">
          {Array.from({ length: 7 }).map((_, i) => (
            <React.Fragment key={i}>
              {i > 0 && <span className={`chain-link chain-link-${i === 0 ? 'current' : 'todo'}`}></span>}
              <span className={`chain-dot chain-dot-${i === 0 ? 'current' : 'todo'}`}></span>
            </React.Fragment>
          ))}
        </div>
...
/* ProcessingOverlay
   props:
     ...
     goalLabel?: string — shown in reveal meta
     onComplete: () => void           — called when stages finish (no reveal)
     reveal?: boolean                 — if true, show the action reveal after stages
     onFinish?: () => void            — called from the reveal "Let's go" button
     finishLabel?: string             — reveal button label (default "Let's go")
*/
function ProcessingOverlay({ source, goalLabel, goalPills, onComplete, reveal, onFinish, finishLabel }) {
      ...
      <div className="proc-overlay">
        <div className="proc-reveal">
          ...
          <button className="btn btn-accent proc-finish-btn" onClick={() => onFinish && onFinish()}>
            {finishLabel || "Let's go"} <Ico.arrow width={13} height={13} />
          </button>
        </div>
      </div>
    );
        ...
        <div className="proc-source">
          <div className="thumb-mini">
            {source && source.type === 'article'
              ? <Ico.external width={16} height={16} />
...
Object.assign(window, { ProcessingOverlay, RevealActionCard, PROC_STAGES });
