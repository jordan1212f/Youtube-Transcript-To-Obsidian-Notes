...
def build_related_section(linked_notes):
    ...
    lines.append('')
    return '\n'.join(lines)
    #! Fix: em dash is not a regular hyphen should be —
...
def append_backlink(filepath, new_note_title):
    ...
    """
    #! Issue: Neither condition handles case where NO related notes section exists (missing Else:)
    path = Path(filepath)
